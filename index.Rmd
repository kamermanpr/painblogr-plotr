...
p('Only numeric variables from the uploaded data are listed as possible choices for the response variable.')
...
output$box_yvar <- renderUI({
    varSelectInput(inputId = 'box_y_var',
                   ...
                   selected = NULL,
                   data = box_df() %>% select(where(is.numeric)))
    })
...
output$box_plot <- renderPlot(
    if(req(input$box_plot_type) == 'box' & req(input$box_group_var) != 'None'){
        ...
        if(req(input$box_group2_var) == 'None'){
            ...
            box_df() %>%
                ggplot(data = .) +
                aes(x = factor(box_df()[[input$box_group_var]]),
                    y = !!input$box_y_var) +
                geom_boxplot() +
            ...
            if(input$box_legend_position != 'legend_user'){
                ...
                box_df() %>% 
                    ...
                    aes(x = factor(box_df()[[input$box_group_var]]),
                        y = !!input$box_y_var,
                        fill = factor(box_df()[[input$box_group2_var]]),
                        colour = factor(box_df()[[input$box_group2_var]])) +
...
p('Only numeric variables from the uploaded data are listed as possible rchoices for the response variables.')

p('For the optional grouping variable, all variables in the uploaded dataset are coerced to categorical data type.')
...
output$scatter_yvar <- renderUI({
    varSelectInput(inputId = 'scatter_y_var',
                   ...
                   data = scatter_df() %>% select(where(is.numeric)))
    })
    ...
    if(req(input$scatter_smooth) == 'none' & req(input$scatter_group_var) == 'None'){
        ...
        if(input$scatter_legend_position != 'legend_user'){
            ...
            ggplot(data = scatter_df()) +
                aes(x = !!input$scatter_x_var,
                    y = !!input$scatter_y_var,
                    fill = factor(scatter_df()[[input$scatter_group_var]]),
                    colour = factor(scatter_df()[[input$scatter_group_var]])) +
                geom_point(shape = 21,
                           alpha = input$scatter_alpha,
                           size = input$scatter_size) +
                geom_point(shape = 21,
...
p('Only numeric variables from the uploaded data are listed as possible choices for the data that will be plotted.')
...
p('All variables in the uploaded data are coerced to categorical data type.')
...
output$bar_numeric <- renderUI({
    varSelectInput(inputId = 'bar_columns_numeric',
                   label = 'Select variables to treat as numeric',
                   selected = NULL,
                   multiple = TRUE,
                   data = bar_df())
})

uiOutput('bar_numeric')

# Choose categorical columns
output$bar_grouping <- renderUI({
    varSelectInput(inputId = 'bar_columns_group',
                   label = 'Select variables to treat as grouping (categorical)',
                   selected = NULL,
                   multiple = TRUE,
                   data = bar_df())
})

uiOutput('bar_grouping')

# Convert columns
bar_df2 <- reactive({
    bar_df() %>% 
        mutate(across(.cols = input$bar_columns_numeric, ~as.numeric(.x))) %>% 
        mutate(across(.cols = input$bar_columns_group, ~factor(.x)))
})

# Select variables
## Response variable
output$bar_analysisvar <- renderUI({
    req(bar_df2())
    varSelectInput(inputId = 'bar_analysis_var',
                   label = 'Choose variable to plot',
                   selected = NULL,
                   data = bar_df())
    })

uiOutput('bar_analysisvar')

## Grouping variable
output$bar_groupvar<- renderUI({
    req(bar_df2())
    selectInput(inputId = 'bar_group_var',
                label = 'Choose grouping variable (optional)',
                selected = 'None',
                choices = c('None', names(bar_df())))
    })

uiOutput('bar_groupvar')
...
output$bar_plot <- renderPlot(
    ...
    if(req(input$bar_analysis_var) != NULL & req(input$bar_group_var) == 'None'){
        
        ggplot(data = bar_df()) +
        aes(x = !!input$bar_analysis_var) +
    ...
    } else if(input$bar_smooth == 'linear' & input$bar_group_var == 'None'){
        
        ggplot(data = bar_df()) +
        aes(x = !!input$bar_x_var,
            y = !!input$bar_y_var) +
        geom_point(shape = 21,
                   colour = '#000000',
                   fill = '#000000',
        ...
        labs(title = input$bar_title,
             y = input$bar_y_label,
             x = input$bar_x_label) +
        scale_x_continuous(limits = c(input$bar_min_x, input$bar_max_x)) +
        scale_y_continuous(limits = c(input$bar_min_y, input$bar_max_y)) +
        theme_minimal(base_size = input$bar_base_size) +
        theme(plot.background = element_rect(colour = '#FFFFFF',
                                             fill = '#FFFFFF'),
    ...
    } else if(input$bar_smooth == 'loess' & input$bar_group_var == 'None'){
        ...
        if(input$bar_legend_position != 'legend_user'){
            ...
            ggplot(data = bar_df()) +
                ...
                theme(plot.background = element_rect(colour = '#FFFFFF',
                                                     fill = '#FFFFFF'),
                      plot.title = element_text(size = input$bar_base_size),
                      legend.position = input$bar_legend_position,
                      legend.title = element_blank(),
                      axis.text = element_text(colour = '#000000'),
                      axis.line = element_line(colour = '#000000',
                                               size = 0.5),
            ...
            ggplot(data = bar_df()) +
                ...
                theme(plot.background = element_rect(colour = '#FFFFFF',
                                                     fill = '#FFFFFF'),
                      plot.title = element_text(size = input$bar_base_size),
                      legend.position = c(input$bar_x_legend, input$bar_y_legend),
                      legend.title = element_blank(),
                      axis.text = element_text(colour = '#000000'),
                      axis.line = element_line(colour = '#000000',
                                               size = 0.5),
