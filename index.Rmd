...
Only comma-separated values (CSV) and MS Excel (XLSX and XLS) files can be uploaded. The first row of the file must be column headings and the data must be "tidy". 
...
The development of the app was inspired by [BoxPlotR](http://shiny.chemgrid.org/boxplotr/){ target='_blank' }. 

Please cite this app as: Kamerman PR, Nair K (2022). plotr: an online academic plotting application. Version: 0.1.5. DOI: [10.5281/zenodo.5977332](https://doi.org/10.5281/zenodo.5977332).
...
p('The default setting is to display plots with no grouping variable using a black-and-white colour palette. Colour options only become available when adding a grouping variable.')
...
output$scatter_xvar <- renderUI({
    ...
    varSelectInput(inputId = 'scatter_x_var',
                   label = 'Choose independent variable (x-axis)',
                   selected = NULL,
                   data =  df_numeric())
    })
...
p('The default setting is to display plots with no grouping variable using a black-and-white colour palette. If you wish to add colour to these plots, select the grouping variable as the same as the primary variable that is being plotted.')
...
radioButtons(inputId = 'bar_na',
             label = 'Grouping variable has missing values',
             choices = c("Keep missing values (default, missing values indicated by 'NA' group')" = 'keep',
                         'Remove missing values' = 'remove'),
             selected = 'keep')
